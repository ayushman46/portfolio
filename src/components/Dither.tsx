...
// --- (All shader code remains exactly the same) ---
...
function DitheredWaves({
  ...
  // This useMemo hook was the fix for the blank screen. It is correct.
  const waveUniforms = useMemo(() => ({
    time: new THREE.Uniform(0),
    resolution: new THREE.Uniform(new THREE.Vector2(size.width * gl.getPixelRatio(), size.height * gl.getPixelRatio())),
  ...
  // --- THIS IS THE FIX ---
  // We are reverting to the original, correct function which uses
  // `gl.domElement` to get the canvas's position.
  const handlePointerMove = (e: ThreeEvent<PointerEvent>) => {
    if (!enableMouseInteraction) return;
    
    // This correctly gets the bounds of the canvas element
    const rect = gl.domElement.getBoundingClientRect();
    const dpr = gl.getPixelRatio();
    
    // This calculates the mouse position *relative to the canvas*
